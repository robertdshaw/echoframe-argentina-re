...
def _cors_headers_for(request) -> Dict[str, str]:
    """
    Compute the CORS headers an error response needs.

    Exception responses generated by exception_handler in newer Starlette
    versions don't always pass back through CORSMiddleware, which means a
    500 ships without Access-Control-Allow-Origin. The browser then masks
    the actual server error as a CORS failure ("No 'Access-Control-Allow-
    Origin' header is present"). We attach the headers explicitly here
    so the real status code and message are visible client-side.
    """
